...
- **랜딩·온보딩 (v0.9.2)**: 첫 화면 = 히어로 + 큰 드롭존 + 가치칩(초보 친화).
  ...
  가이드 모달(localStorage oncut_seen), ❔사용법 버튼으로 재열기. "편집기 보고
  포기" 방지.
- **시간별 표시(v0.9.1)**: 텍스트·버튼 오버레이는 표시구간 지나면 편집기에서도
  사라짐(선택 중엔 흐리게). 클립 길이 인스펙터 숫자+슬라이더. 효과음 미리듣기.
...
- **NLE-lite 편집기 (v0.4.0~, 기본 탑재)**:
  ...
  - **멀티소스 짜깁기 (v0.12.0)**: 영상+영상, 영상+이미지(사진)를 한 타임라인에
    이어붙이기. 클립에 `src`(소스 토큰), 이미지는 -loop+무음(aevalsrc). 모든 클립을
    공통 캔버스로 scale+crop+fps 정규화해 concat/xfade 안전. /api/addsource로 소스
    추가, /api/media?src=토큰 서빙. 편집기 재생은 소스 전환(영상)·정지이미지(사진).
